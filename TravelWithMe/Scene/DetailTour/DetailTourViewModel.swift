...
class DetailTourViewModel: ViewModelType {
    ...
    var tourItemsDelegate1: TourItemsProtocol1?
...
//    var wholeContentsViewModel: ContentsViewModel? = nil
    ...
    func updateDelegate1Data(_ likeStatus: Bool) {

        do {
            var values = try tourItemsDelegate1?.tourItems.value() ?? []
            ...
            tourItemsDelegate1?.tourItems.onNext(values)
        ...
        input.applyButtonClicked
            ...
            .map { [weak self] response in
                ...
                case .success(let result):
                    ...
                    // (2). 이전 VC에서 물고있는 데이터 변경해주기
                        // 프로토콜 중 nil이 아닌 곳
                    if self?.tourItemsDelegate1 != nil {
                        print("tourItemProtocol1이 nil이 아님")
                        self?.updateDelegate1Data(result.like_status)
